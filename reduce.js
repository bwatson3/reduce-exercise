...
function vowelCount(str) {
	...
	return str.split('').reduce(function(accum, nextVal) {
		let sameCase = nextVal.toLowerCase();
		if (vowels.indexOf(sameCase) !== -1) {
			if (accum[sameCase]) {
				accum[sameCase]++;
			} else {
				accum[sameCase] = 1;
...
should run the callback function on each value in the array and if the result
of the callback function at that specific value is true, the value should be
